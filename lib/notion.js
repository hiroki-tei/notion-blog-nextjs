...
export const listLabelsFromCategory = cache(async (category) => {
  ...
  const pages = firstResponse.results
  const labels = pages.map(res => {
    return {
      name: res.properties['名前'].title[0].plain_text,
      articles: res.properties['アウトプット記事'].relation.map(rel => rel.id) // array of pageID
    }
  }).flat()
  return labels
});
...
export const getPagesWithURI = cache(async (pages) => {
  const pageResults = await Promise.all(pages.map(page => getPage(page)))
  return pageResults
    .filter(page => {
